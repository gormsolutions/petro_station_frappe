...
class GasInvoices(Document):
	...
	def create_stock_entry(self, stock_entry_type, gas_sale_id_field, success_message, items):
		...
		for item in items:
			rate = frappe.db.get_value("Item Price", {
				...
				"price_list": self.price_list,  # Use your Buying price list
				"buying": 1
			}, "price_list_rate") or 0

			 # If no rate in Item Price, get the latest rate from submitted Purchase Invoices
			if not rate:
				rate = frappe.db.sql("""
					SELECT rate
			...
			stock_entry.append("items", {
				...
				"basic_rate": rate,       # Critical for custom valuation
        		"valuation_rate": rate,   # Optional but ensures correct stock value
			})
	...
	def create_sales_invoice(self):
		...
		sales_invoice.custom_invoice_no = self.prn
