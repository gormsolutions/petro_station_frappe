...
frappe.ui.form.on('Station Shift Management', {
    ...
    get_all_shift_details: function (frm) {
        ...
        populateInvoiceItems(frm);
        populateInvoiceItemsOutstanding(frm);
        populateExpenditureTable344(frm);
        populateCashTransferTable(frm);
    ...
    before_submit: function (frm) {
        ...
        //     // Array to store promises for validation
        //     let validations = frm.doc.overal_shift_closing_items.map((item, i) => {
        //         return new Promise((resolve, reject) => {
        //             // Get the warehouse type for the current item.pump_or_tank
        //             frappe.db.get_value('Warehouse', item.pump_or_tank, 'warehouse_type', (r) => {
        //                 if (r && r.warehouse_type !== 'Store') {
        ...
        //                     let current_reading = item.current_reading_value;
        //                     let qty_based_on_sales = item.qty_on_sales;
        //                     let qty_sold_on_meter_reading = item.qty_on_meter;
        //                     let sales_based_on_invoices = item.sales_based_total_amount;
        //                     let sales_based_on_meter_reading = item.meter_based_total_amount;

        //                     // Tolerance for floating-point comparison
        //                     let tolerance = 10000.000;
        ...
        //                     if (rate === 0) {
        //                         // Notify the user and prevent saving
        //                         frappe.msgprint(__('Row {0}: rate cannot be zero. Please enter a valid rate.', [i + 1]));
        //                         resolve(false);
        //                     }
...
function getBankingandCash(frm) {
    ...
    frappe.call({
        method: 'petro_station_app.custom_api.api.get_gl_account',
        args: {
            station: frm.doc.station,
            from_date: frm.doc.from_date,
            employee: frm.doc.employee,
    ...
    frappe.call({
        ...
        callback: function (r) {
            if (r.message && r.message.Invoices) {
                ...
                // Clear existing items in 'credits_customer_item' table
                frm.clear_table('credits_customer_item');
                ...
                invoices.forEach(function (invoice) {
                    ...
                    items.forEach(function (item) {
                        // Add a new child row to 'credits_customer_item'
                        var new_item = frm.add_child('credits_customer_item');

                        // Set values from invoice and item to the new child row fields
                        new_item.invoice = invoice['Invoice Name'];
                        new_item.invoice_no = invoice['Invoice No'];
                        new_item.posting_date = invoice['Posting Date'];
                        new_item.customer_name = invoice['Customer Name'];
                        new_item.customer = invoice['Customer'];
                        new_item.outstanding_amount = invoice['Outstanding Amount (UGX)'];
                        // Add other relevant fields based on your data structure

                    });
                });

                // Refresh the child table view after all items have been added
                frm.refresh_field('credits_customer_item');
            } else {
                frappe.msgprint("No valid invoices found in response.");
            }
        },
